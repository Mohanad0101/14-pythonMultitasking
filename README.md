# Лабораторная 14: Основы многозадачного программирования   Часть 1

**Курс:** Сетевое системное программирование  
**Тема:** TCP‑сервер с несколькими клиентами, параллельный сканер портов

**Источник:** [Многозадачное программирование](https://koroteev.site/text/os34-0/) (М. Коротеев).
...
*Рисунки лежат в папке `images/` рядом с этим файлом.*
...
**Пример:** два потока снимают деньги с одного счёта. Баланс 50; каждый хочет снять 30. Логика: «если баланс ≥ сумма, вычесть сумму». Оба потока проходят проверку (50 ≥ 30) и оба вычитают 30. Порядок выполнения не фиксирован: итоговый баланс может стать 20 (если оба записали 20, последняя запись побеждает) или −10 (если один поток успел прочитать уже обновлённое значение 20 и вычесть из него 30).
...
sequenceDiagram
  ...
  Note over Bal: Wrong: 20 (or -10 if order differs)
```
...
**Заготовка (дополните TODO в `main()`):**
...
1. Файл `answers.txt`   письменные ответы и наблюдения для всех вопросов, а также результаты запусков (см. §6.4).  
2. **Работающий многопоточный сервер**, который обслуживает несколько клиентов одновременно, и **работающий параллельный сканер портов**, который показывает открытые порты и время работы. Преподаватель убедится в этом при запуске ваших программ.
...
Важно не только, чтобы код работал, но и чтобы вы понимали, **как** именно используются потоки, процессы и asyncio.
